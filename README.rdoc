...
6. Save payer_email to send order confirmation and fulfillment emails.
...
# Write a test for the following line of fraud check. Instead of order total, payment.amount is checked.
...
#   
#       Item can have many orders
#       Find a unique order in the db by searching on custom variable : 
# 			order = Order.find_by_custom(notify.custom)
# 
